  ...
  <v-container>
    ...
    <v-flex sm8 offset-sm-2>
      <v-card flat>
          ...
          <div v-if="$vuetify.breakpoint.smAndUp" class="d-flex justify-space-around">
              ...
              <div>
                <v-chip
                  ...
                  text-color="white"
                  class="count"
                  v-if="cntRentableBook !== '0권' && cntRentableBook !== '미보유'"
                >
                  {{cityIdtocityName(this.user.cityId)}} {{cntRentableBook}}
                </v-chip>
                <v-chip
                  outlined 
                  color="white" 
                  ...
                  class="count"
                  v-if="cntRentableBook === '0권' || cntRentableBook === '미보유'"
                  :style="textCenter(cntRentableBook)"
        ...
        <v-card class="mb-5" :min-height="swiperMinHeight" flat tile>
          <v-container class="relateBookDesign">
            <p class="black--text headline mb-1" v-if="$vuetify.breakpoint.smAndUp">관련 도서<v-icon size="24px" style="position: absolute; right:0px; top:5px;">fa-chevron-right</v-icon></p>
            <p class="title font-weight-bold mb-1" v-else>관련 도서<v-icon size="24px" style="position: absolute; right:0px; top:5px;">fa-chevron-right</v-icon></p>
            <div class="swiper-container relate">
              <div class="swiper-wrapper">
                <div class="swiper-slide" v-for="(categoryBook, idx) in categoryBooks" :key="idx" :style="sliderHeight">
                  <router-link :to="{
                    name: 'bookdetail',
                    query: {
                      bookISBN: categoryBook.bookISBN,
                      categoryId: categoryBook.categoryId
                    }
                  }">
                    <div v-if="$vuetify.breakpoint.smAndUp">
                      <v-card tile :elevation="6" outlined>
                      <v-img
                        height="170px"
                        width="120px"
                        :src="categoryBook.bookImg"
                        alt="categoryBookImg"
                      ></v-img>
                      </v-card>
                    </div>
        ...
        <div class="ma-3">
          <v-card flat>
            <div class="mt-10">
              <p class="mb-0 title" v-if="$vuetify.breakpoint.smAndUp"><v-icon class="mr-1">{{ icons.mdiCommentMultipleOutline }}</v-icon>리뷰</p>
              <p class="mb-0" v-else><v-icon class="mr-1">{{ icons.mdiCommentMultipleOutline }}</v-icon>리뷰</p>
            </div>
            <v-layout>
              <v-flex>
                <v-card flat>
                      ...
                      <v-col cols="10">
                        <form @submit.prevent="regComment">
                          ...
                          <div class="d-flex justify-end" v-if="$vuetify.breakpoint.smAndUp">
                            <v-btn tile depressed class="white--text" type="submit" color="black">등록</v-btn>
                          </div>
                          <div class="d-flex justify-end" v-else>
                            <v-btn tile depressed class="white--text" type="submit" small color="black">등록</v-btn>
                          </div>
                        </form>
                      </v-col>
...
<script>
  ...
  export default {
    ...
    computed: {
      sliderHeight() {
        if (this.$vuetify.breakpoint.smAndUp) {
          return `height: ${this.sliderSize * 1.8}px`
        }
        else {
          return `height: ${this.sliderSize}px`
        }
      },
      swiperMinHeight() {
    ...
    methods: {
      ...
      addRentMark() {
        this.getAxios.get(`/book/rent/mark/${this.$route.query.bookISBN}`, this.requestHeader)
          .then(res => {
            alert("도서 반납 알림 메세지를 받습니다.")
          })
          .catch(err => {
            console.log(err)
          })
      },
      getCountRentableBook() {
        this.getAxios.get(`/book/rentable/count/${this.$route.query.bookISBN}`, this.requestHeader)
          .then(res => {
            this.cntRentableBook = res.data.cntRentableBook
            this.markable = res.data.markable
          })
          .catch(err => {
            console.log(err)
          })
      ...
      editCmnt() {
        this.getAxios.post(`/book/update/${this.selectedComment.bookCommentId}`, {
          bookCommentContent: this.content,
          bookISBN: this.bookInfo.bookISBN,
          bookCommentWriter: this.user.userId
        }, this.requestHeader)
          .then(res => {
      ...
      mainInit() {
        ...
        this.getAxios.get(`/book/category/${this.$route.query.categoryId}`, this.requestHeader)
        .then(res => {
          this.categoryBooks = res.data.categoryBook
   
          requestAnimationFrame(function() {
            const bestSwiper = new Swiper ('.relate', {
              // Optional parameters
              direction: 'horizontal',
              slidesPerView: 3.4,
              spaceBetween: 30,
      ...
      textCenter(cntRentableBook) {
        if (cntRentableBook === '미보유') {
          return 'padding-left: 0.8em;'
        }
      }
    },
    created() {
